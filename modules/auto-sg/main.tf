...
resource "aws_launch_template" "public_launch_template" {
  ...
  key_name  = "Template-Keys"
  user_data =  filebase64("../modules/auto-sg/ec2init.sh")
...
resource "aws_launch_template" "private_launch_template" {
  ...
  image_id      = "ami-0acb327475c6fd498"
  instance_type = "t2.micro"
  key_name  = "Template-Keys"
...
###Attaching it to auto scaloing group. 
...
resource "aws_autoscaling_group" "public_autoscaling_group" {
  ...
  vpc_zone_identifier = [var.public_subnet_2a_id, var.public_subnet_2b_id]
